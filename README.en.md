...
idle_speed: normal       # slow | normal | fast | off (static, rests on the first palette = red)
idle_theme: auto         # tints the accent colour only; no longer affects the background
```

- The **12 fixed palettes** (starting from **red**) are **red / teal / orange / lake teal / gold / indigo / warm brown / wisteria / pine ink / magenta / grass green / slate grey**, hard-coded in the card and independent of the theme.
...
- **The starting point only sets the "first palette seen"**: the ring is cyclic, so rotating the start **changes no adjacent ΔE at all** (the sum is invariant too) — it only decides which colour appears first when the card enters the idle state. Red is the start because it is the highest-chroma, most saturated warm hue on the ring, giving the strongest opening recognition. To change the first palette, just rotate the start of `IDLE_GROUPS` — no need to re-run the ring optimisation.
...
- `idle_speed` scales the **seconds each palette holds**: `slow` 11.2s / `normal` 7s / `fast` 4.2s, i.e. a full cycle of 89.6s / 56s / 33.6s; `off` rests on the first palette (red).
