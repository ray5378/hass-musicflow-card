...
// 12 组固定配色。顺序是一条**固定的环**(代码里没有任何随机数),以**红**为起点:
//   红 → 青绿 → 橙 → 湖青 → 黄 → 靛蓝 → 暖褐 → 藤紫 → 松墨 → 品红 → 草绿 → 烟灰 →(回红)
// 环的顺序用 Lab ΔE 直接优化(max-min),结果 **相邻两组的最小 ΔE = 65.7** ——
//   每换一组都是一次肉眼明确的跳变(ΔE 65 约等于"中蓝 → 中橙"的差距)。
// 起点选红:环是循环的,**旋转起点不改变任何一段相邻 ΔE**,只决定"首次进入 idle 态时先看到哪个
//   颜色"(即首屏色)。红是这条环上彩度最高、色相最正的暖色,开场辨识度最高。
...
const IDLE_GROUPS = [
  { c1: "#b2262f", c2: "#9a3424" }, // 红 深红  Y .110   ← 起点(开场色)
