...
class MusicFlowPlayerCard extends LitElement {
  ...
  /* ==================== 计算属性 / helpers ==================== */
  get _entity() { return this.hass?.states?.[this._activeEntity]; }
  get _attr() { return this._entity?.attributes || {}; }
  get _state() { return this._entity?.state; }
  ...
  _switchPeer(newEntity) {
    ...
    /* 主项目 switchPeer 纯 UI 操作:只切换控制目标,旧播放器的队列/状态不动。
     每个 peer 已是独立 HA 实体,WS 状态推送独立,无需任何服务器/集成调用。 */
    this._showPlayerPicker = false;
      ...
      <ha-card class="card">
        ...
        <div class="controls">
          ...
          <button class="action" title="${this._playing ? "暂停" : "播放"}" @click=${this._togglePlay}>
            ${icon(this._playing ? "pause" : "play", 30)}
          </button>
