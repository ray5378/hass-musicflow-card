...
当前播放器**没有封面可显示**时（停止 / 清空队列 / 无媒体），卡片改用「5 组固定配色轮换」的底色填充，避免整卡失去颜色。有封面时一律沿用封面主色，两者互斥。
...
因为 5 层相位恰好错开 20%，**任一时刻各层 opacity 之和恒为 1**：交叉区里必然一层在降、另一层在升，所以既不会亮度塌陷，也没有「先暗再亮」的空档。全程只动 `opacity`（合成属性）——**无位移、无重绘、无滤镜，主线程零参与**。
...
- 5 组固定配色：**靛蓝 / 暖褐 / 松墨 / 藤紫 / 湖青**，写死在卡片里，不随主题变化。
- 这 5 组按**亮度 V 形**排序（峰 → 中 → 谷 → 中 → 峰），峰值到谷底的相对亮度差 **1.88×**。所以整轮读起来是一次明显的「吸气—呼气」明暗起伏，而不只是色相在挪；首尾都是峰值，回绕那一下亮度连续、不会硬跳。
- `idle_speed` 缩放「每组停留秒数」：`slow` 8s / `normal` 5s / `fast` 3s，一轮分别是 40s / 25s / 15s；`off` 静止在首组配色。
