...
idle_speed: normal       # slow | normal | fast | off(纯静态,停在首组＝红)
idle_theme: auto         # 只影响强调色(图标/进度条),不再影响底色
```

- **12 组固定配色**（以**红**为起点）：**红 / 青绿 / 橙 / 湖青 / 黄 / 靛蓝 / 暖褐 / 藤紫 / 松墨 / 品红 / 草绿 / 烟灰**，写死在卡片里，不随主题变化。
...
- **起点只决定「首屏色」**：环是循环的，旋转起点**不改变任何一段相邻 ΔE**（Σ 也恒定不变），只决定首次进入 idle 态时先看到哪个颜色。起点取红，是因为它是这条环上彩度最高、色相最正的暖色，开场辨识度最高。想换首屏色只需旋转 `IDLE_GROUPS` 的起点，不必重跑环序优化。
...
- `idle_speed` 缩放「每组停留秒数」：`slow` 11.2s / `normal` 7s / `fast` 4.2s，一轮分别是 89.6s / 56s / 33.6s；`off` 静止在首组（红）。
...
- 卡片滚出视口、切到后台标签页、打开队列/媒体库面板时自动停表；系统开启"减少动态效果"时降级为静态底色（停在首组＝红）。
