...
idle_speed: normal       # slow | normal | fast | off(纯静态,停在首组配色)
idle_theme: auto         # 只影响强调色(图标/进度条),不再影响底色
```

- **12 组固定配色**：**品红 / 草绿 / 烟灰 / 红 / 青绿 / 橙 / 湖青 / 黄 / 靛蓝 / 暖褐 / 藤紫 / 松墨**，写死在卡片里，不随主题变化。
- **顺序是固定的环，不随机**：代码里没有任何随机数，永远按上面这个顺序循环。环的顺序用 **Lab ΔE 直接优化**（max-min 判据），最终**相邻两组的最小 ΔE = 65.7** —— 每换一组都是一次肉眼明确的跳变（ΔE 65 约等于「中蓝 → 中橙」的差距）。
- 判「两组看着像不像」的口径是 **Lab ΔE\*ab，不是色相差**：低彩度的深青绿必须拉开很大色相才分得开。初版 12 组里「松墨 vs 青绿」只有 ΔE 15~17，于是把松墨挪到 125°（真·墨绿）、青绿提彩度到 65% 并挪到 170°、暖褐降饱和 —— 三者最近邻距离由 15/20/21 拉到 27/27/24。全表最像的一对是「暖褐 vs 橙」（ΔE 24.4），它们在环上不相邻。
- 每组按**相对亮度 Y** 定标（不是 HSL 的 L）：`峰 0.175`（草绿 / 湖青 / 靛蓝）/ `0.140`（品红 / 青绿 / 橙 / 黄）/ `0.110`（红 / 暖褐 / 藤紫）/ `0.085`（烟灰）/ `谷 0.070`（松墨），峰值到谷底 **1.88×**。相邻两组都同时吃到「换色 + 换明暗」两重变化，观感上的「呼吸」才立得住。
- 红 / 黄 / 橙 受「暗底 + 白字」约束（相对亮度上限 ≈0.183，再亮白字对比度就跌破 WCAG AA），所以呈现为**深红 / 焦橙 / 暗金**，不是明亮原色 —— 这是取舍，不是取色失误。
- 烟灰是唯一的**无彩**组（饱和度 ≈7%），补的是「色相之外的那条轴」，夹在两张高彩之间当一次换气。
- `idle_speed` 缩放「每组停留秒数」：`slow` 11.2s / `normal` 7s / `fast` 4.2s，一轮分别是 89.6s / 56s / 33.6s；`off` 静止在首组配色。
...
- 卡片滚出视口、切到后台标签页、打开队列/媒体库面板时自动停表；系统开启"减少动态效果"时降级为静态底色（停在首组配色）。
