...
当前播放器**没有封面可显示**时（停止 / 清空队列 / 无媒体），卡片改用「8 组固定配色轮换」的底色填充，避免整卡失去颜色。有封面时一律沿用封面主色，两者互斥。

实现是 **5 层整卡对角渐变、原地交叉淡入**：每层一条 `linear-gradient(140deg, c1, c2 62%, #14182a 100%)`，初始 `opacity: 0`，各自跑同一条关键帧，靠**负 `animation-delay`** 把 5 层相位均匀错开 1/5 周期。

关键帧把「停留」与「交叉」固定在一轮的固定区间里：

```css
@keyframes mf-cyc {
  0%, 10%  { opacity: 1; }  /* 前 1/10 全亮停留 */
  20%, 90% { opacity: 0; }  /* 10%~20% 交叉淡出，20%~90% 全透 */
  100%     { opacity: 1; }  /* 最后 1/10 淡入回来 */
}
```

各层相位均匀错开 1/N 周期（N = 配色组数，现 **8 组**）——交叉区里必然一层在升、另一层在降，所以既没有「先暗再亮」的空档，也不会叠加变亮（每层都是**全不透明**渐变，`over` 合成会自动归一）。唯一要留意的是**底层漏出**：所有层都半透时会透出卡片底色。实测最大漏出 **N=5 → 25%**，**N=8 → 1.56%**（错峰更密，几乎总有一层接近满不透明压在最下面，此时任一层最高不透明度最低也有 87.5%）。全程只动 `opacity`（合成属性）——**无位移、无重绘、无滤镜，主线程零参与**。

> 2.3.0 用的是「5 格雪碧条 + `background-position` 平移」，过渡是相邻两格的空间线性插值，底色会跟着「往左走」而不是原地换色；**2.3.1 已改为 A+ 方案的原地交叉淡入**。
...
- 8 组固定配色：**靛蓝 / 暖褐 / 湖青 / 黄 / 藤紫 / 橙 / 松墨 / 红**，写死在卡片里，不随主题变化。
- 这 8 组排成一个环，**相邻两组既不撞色相、也不撞明暗**：每段色相差 ≥118°、相邻亮度比 ≥1.18×。所以每换一组都同时吃到「换色 + 换明暗」两重变化，观感上的「呼吸」才立得住。相对亮度按 `峰 0.175 / 0.140 / 0.110 / 谷 0.070` 定标，峰值到谷底 **1.88×**。
- 红 / 黄 / 橙 受「暗底 + 白字」约束（亮度上限 ≈0.183，再亮白字对比度就跌破 WCAG AA），所以呈现为**深红 / 暗金 / 焦橙**，不是明亮原色。
...
- 底色是固定深色渐变（收尾 `#14182a`），所以 idle 态一律按「暗底浅字」渲染，不再随 HA 明暗模式切浅色底。
