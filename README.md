...
当前播放器**没有封面可显示**时（停止 / 清空队列 / 无媒体），卡片改用「5 组配色轮换」的流动底色填充，避免整卡失去颜色。有封面时一律沿用封面主色，两者互斥。

底层是**一张 5 格横条雪碧图（sprite）**：`background-size: 500%` 让一格刚好等于卡宽，`background-position` 沿横条平移一格 = 换一组配色。`background-position` 与 `transform` / `opacity` 同属合成属性，**全程不产生 Paint / Raster / Layout**——动画主线程零参与。

三层结构：

- `base`：由 HA 主题色 `--primary-color` 派生的低饱和底色，**永远不动**，承担白字对比度；底色层不换色，换色期间不存在亮度塌陷。
- `sprite`：5 组配色横条，按 `idle_speed` 节奏平移换格；相邻两组仅差 22° 色相，一圈后回绕，全部由单一色相派生。
- `breath`：一层柔光晕，缓慢位移 / 缩放 / 明暗（`transform` + `opacity`），与换色节奏相互独立。
...
- `idle_theme`：冷启动（HA 重启后还没播过歌）没有缓存色时的回落。`auto`（默认）取 HA 主题的 `--primary-color` 派生；也可固定 `twilight` / `ocean` / `ember` / `forest` / `mono`，改为只固定色相。播过一首歌之后这个配置就不再起作用。
- `idle_speed` 同时驱动呼吸与换色：`slow` 32s / `normal` 20s / `fast` 12s / `off` 静止在首组配色（一轮分别为 160s / 100s / 60s）。**`off` 档观感变了**：旧实现停动画后仍可见 3 个光斑，新实现静止在首组配色。
- 卡片滚出视口、切到后台标签页、打开队列/媒体库面板时自动停表；系统开启"减少动态效果"时降级为静态底色。
