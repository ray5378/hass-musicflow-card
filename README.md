...
- 5 组固定配色：**暮蓝 / 暖褐 / 青苔 / 紫灰 / 鴨青**，写死在卡片里，不随主题变化。
- `idle_speed` 缩放「每组停留秒数」：`slow` 32s / `normal` 20s / `fast` 12s，一轮分别是 160s / 100s / 60s；`off` 静止在首组配色。**`off` 档观感变了**：旧实现停动画后仍可见 3 个光斑，新实现静止在首组配色。
